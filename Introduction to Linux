...
Subject: CSE1CPR – Cybersecurity in Practice Environment: Kali Linux (VM), BASH shell
...
File system navigation and management Built and manipulated a nested test directory structure (test-dir-10/test-subdir-A/test-subdir-B/...) to practice absolute vs. relative paths. Used ls, mkdir, cd, mv, and cp to create, list, rename, and reorganise directories and files.

Finding and handling awkward files Worked through several file-discovery and file-handling scenarios that come up often in Linux/security work:
...
Bandit CTF (OverTheWire) Connected to the Bandit wargame server over SSH and progressed through several levels, using each level's hints to locate the password needed to access the next. This put the day's commands into practice in a "find the hidden thing" format — reinforcing ls -la, cat, find, and handling special characters in filenames under realistic (if intentionally awkward) conditions.
